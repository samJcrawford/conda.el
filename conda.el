...
Cached for the lifetime of the process."
  (with-memoization conda--executable-path
    (or
...
Cached for the lifetime of the process."
  (with-memoization conda--installed-version
    (condition-case err
...
(defun conda--update-env-from-params (params)
  ...
  (let ((exports (or (conda-env-params-vars-export params) '())))
    ...
    (let ((unsets (or (conda-env-params-vars-unset params) '())))
      (mapc (lambda (arg)
              (insert (format "About to unset %s\n" arg))
...
updated PATH."
  (with-memoization (gethash env-dir conda--activation-parameters)
	...
	(let ((result (conda--call-json-subcommand "activate" env-dir)))
      ...
      (if (conda--supports-old-activate-format)
	  (make-conda-env-params
	   :path (concat
...
(defun conda-env-activate-path (path)
  ...
  (unless (conda--env-dir-is-valid path)
  ...
  (let* ((env-dir (expand-file-name path))
    ...
    (let ((params (conda--get-activation-parameters env-dir)))
      (if (conda-env-params-vars-export params)
          (conda--update-env-from-params params)
        (progn ;; otherwise we fall back to legacy heuristics
  ...
  (when (or conda-message-on-environment-switch (called-interactively-p 'interactive))
